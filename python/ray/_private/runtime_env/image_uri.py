...
def _modify_context_impl(
    image_uri: str,
    container_options: Optional[List[str]],
    ...
    if container_options:
        container_command.extend(container_options)
...
class ImageURIPlugin(RuntimeEnvPlugin):
    ...
    def modify_context(
        ...
        if not runtime_env.get("image_uri"):
            return

        _modify_context_impl(
            runtime_env["image_uri"], [], context, logger, self._ray_tmp_dir
        )
...
class ContainerPlugin(RuntimeEnvPlugin):
    ...
    def modify_context(
        ...
        worker_path = runtime_env["container"].get("worker_path")
        if worker_path:
