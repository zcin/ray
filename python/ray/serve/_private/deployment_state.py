...
class DeploymentState:
    ...
    def _stop_or_update_outdated_version_replicas(self, max_to_stop=math.inf) -> bool:
        ...
        print(
            "replicas to update",
            [(r.replica_tag, r.actor_details.state) for r in replicas_to_update],
        )
    ...
    def _migrate_replicas_on_draining_nodes(self):
        ...
        num_running = self._replicas.count(
            states=[ReplicaState.RUNNING], version=self._target_state.version
        )
        num_draining = self._replicas.count(
            states=[ReplicaState.PENDING_MIGRATION], version=self._target_state.version
        )
