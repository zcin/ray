...
class MetricsPusher:
    ...
    def __init__(
        ...
        sleep_coroutine: Callable = asyncio.sleep,
    ):
        self._event_loop = event_loop
        self._sleep_coroutine = sleep_coroutine
        self._tasks: Dict[str, asyncio.Task] = dict()
        self._stop_event = asyncio.Event(loop=event_loop)
        self._wait_on_stop_event = self._event_loop.create_task(self._stop_event.wait())

    def start(self):
        self._stop_event.clear()

    async def metrics_task(self, task_func: Callable, interval_s: float):
        ...
        while True:
            try:
                ...
                logger.exception(f"Failed to run metrics task: {e}")

            await asyncio.wait(
                [
                    self._wait_on_stop_event,
                    asyncio.create_task(self._sleep_coroutine(interval_s)),
                ],
    ...
    def register_or_update_task(
        ...
        self._tasks[name] = self._event_loop.create_task(
            self.metrics_task(task_func, interval_s)
        )
