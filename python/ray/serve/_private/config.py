...
class DeploymentConfig(BaseModel):
    ...
    Args:
        ...
        max_concurrent_requests: The maximum number of queries
...
def handle_num_replicas_auto(
    max_concurrent_requests: Union[int, DEFAULT],
    autoscaling_config: Optional[Union[Dict, AutoscalingConfig, DEFAULT]],
):
    """Return modified `max_concurrent_requests` and `autoscaling_config`
    for when num_replicas="auto".

    If `max_concurrent_requests` is unspecified (DEFAULT.VALUE), returns
    the modified value NEW_DEFAULT_MAX_CONCURRENT_REQUESTS. Otherwise,
    doesn't change `max_concurrent_requests`.
    ...
    if max_concurrent_requests is DEFAULT.VALUE:
        max_concurrent_requests = NEW_DEFAULT_MAX_CONCURRENT_REQUESTS
    ...
    return max_concurrent_requests, autoscaling_config
