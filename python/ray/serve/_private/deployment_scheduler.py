...
class LaunchingReplicaInfo:
    ...
    Args:
        target_az: The availability zone that's been requested for this
...
class DeploymentScheduler(ABC):
    ...
    def _on_replica_launching(
        ...
        self._launching_replicas[deployment_id][replica_name] = LaunchingReplicaInfo(
            target_node_id=target_node_id, target_az=target_az
        )
        ...
        for deployment_id, replicas in self._launching_replicas.items():
            for info in replicas.values():
                if info.target_node_id:
                    available_minus_replicas[info.target_node_id] -= self._deployments[
                        deployment_id
                    ].required_resources
    ...
    def _schedule_replica(
        ...
        self._on_replica_launching(
            deployment_id, replica_name, target_node_id=target_node_id
        )
