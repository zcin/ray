...
class RouterMetricsManager:
    ...
    def __init__(self, deployment_id: DeploymentID, handle_id: str, controller_handle):
        ...
        self.num_queued_requests_gauge = metrics.Gauge(
    ...
    def update_autoscaling_config(
        self, autoscaling_config: AutoscalingConfig, curr_num_replicas: int
    ):
    ...
    def should_send_scaled_to_zero_optimized_push(self, curr_num_replicas: int) -> bool:
        return (
            ...
            and self.num_queued_requests
        )

    def push_metrics_to_controller(self):
    ...
    def _get_aggregated_requests(self):
        ...
        if RAY_SERVE_COLLECT_AUTOSCALING_METRICS_ON_HANDLE:
            ...
            running_requests = {
                ...
                or num_requests
                for replica_id, num_requests in list(
                    self.num_requests_sent_to_replicas.items()
                )
            }
...
class Router:
    ...
    def update_autoscaling_config(self, autoscaling_config: AutoscalingConfig):
        self._metrics_manager.update_autoscaling_config(
            autoscaling_config,
    ...
    async def assign_request(
        ...
        **request_kwargs,
        ...
        self._metrics_manager.inc_num_total_requests(request_meta.route)
        self._metrics_manager.inc_num_queued_requests()
        ...
        if self._metrics_manager.should_send_scaled_to_zero_optimized_push(
            ...
            self._metrics_manager.push_metrics_to_controller()
        ...
        try:
            ...
            if RAY_SERVE_COLLECT_AUTOSCALING_METRICS_ON_HANDLE:
                self._metrics_manager.inc_num_running_requests_for_replica(replica_tag)
                callback = partial(
