...
def test_downscaling_with_fractional_smoothing_factor(serve_instance):
    controller = serve_instance._controller
    ...
    # Deployment should still have 2 replicas since downscale delay = 5
    ...
    # smoothing factor
    wait_for_condition(
        check_autoscale_num_replicas_eq, controller=controller, name="A", target=1
    )
