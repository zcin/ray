...
def test_max_ongoing_requests_set_to_one(serve_instance, use_max_concurrent_queries):
    ...
    if use_max_concurrent_queries:
        h = serve.run(f.options(max_concurrent_queries=1).bind())
    else:
        h = serve.run(f.options(max_ongoing_requests=1).bind())
