...
class TestCompactScheduling:
    ...
    @pytest.mark.parametrize("use_strict_pack_pg", [True, False])
    def test_max_replicas_per_node(self, use_strict_pack_pg: bool):
        ...
        d_id1 = DeploymentID(name="deployment1")
        placement_group_bundles = (
            [{"CPU": 1}, {"CPU": 1}] if use_strict_pack_pg else None
        )
        placement_group_strategy = "STRICT_PACK" if use_strict_pack_pg else None

        cluster_node_info_cache = MockClusterNodeInfoCache()
        cluster_node_info_cache.add_node("node1", {"CPU": 20})
        cluster_node_info_cache.add_node("node2", {"CPU": 10})
