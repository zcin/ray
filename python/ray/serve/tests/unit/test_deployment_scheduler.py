...
def test_downscale_head_node():
    ...
    cluster_node_info_cache = MockClusterNodeInfoCache()
    scheduler = DefaultDeploymentScheduler(
        cluster_node_info_cache, head_node_id=head_node_id, create_placement_group=None
    )
...
class TestCompactScheduling:
    def test_basic(self):
        ...
        def on_scheduled2(actor_handle, placement_group):
    ...
    def test_placement_groups(self):
        ...
        scheduler = DefaultDeploymentScheduler(
            cluster_node_info_cache,
            head_node_id="fake-head-node-id",
            create_placement_group=lambda *args, **kwargs: MockPlacementGroup(
                *args, **kwargs
            ),
        )
        ...
        def on_scheduled(actor_handle, placement_group):
            ...
            assert placement_group._soft_target_node_id == "node2"

        def on_scheduled2(actor_handle, placement_group):
            scheduling_strategy = actor_handle._options["scheduling_strategy"]
            assert isinstance(scheduling_strategy, PlacementGroupSchedulingStrategy)
            assert placement_group._soft_target_node_id == "node1"
