...
class TestRouterMetricsManager:
    ...
    async def test_memory_cleared(self):
        ...
        def check_database(expected: Set[ReplicaID]):
            # Check that the database keys were pruned
            assert set(metrics_manager.metrics_store.data) == expected
            # Check that no key has more than 1 datapoint because the
            # data should be compacted down to 0.01s window
            for datapoints in metrics_manager.metrics_store.data.values():
                assert len(datapoints) == 1
            return True

        # r1: 1
        metrics_manager.inc_num_running_requests_for_replica(r1)
        await async_wait_for_condition(check_database, expected={r1})

        # r1: 1, r2: 0
        metrics_manager.inc_num_running_requests_for_replica(r2)
        await async_wait_for_condition(check_database, expected={r1, r2})
        metrics_manager.dec_num_running_requests_for_replica(r2)

        # r1: 1, r2: 0, r3: 0
        metrics_manager.inc_num_running_requests_for_replica(r3)
        await async_wait_for_condition(check_database, expected={r1, r2, r3})
