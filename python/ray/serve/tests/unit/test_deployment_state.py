...
def test_redeploy_no_version(mock_deployment_state_manager_full):
    ...
    check_counts(
        ds,
        total=2,
        by_state=[(ReplicaState.STOPPING, 1, None), (ReplicaState.STARTING, 1, None)],
    )
    ...
    b_info_3, _ = deployment_info(version="3")
...
def test_redeploy_new_version(mock_deployment_state_manager_full):
    ...
    updating = dsm.deploy(TEST_DEPLOYMENT_ID, b_info_2)
    assert updating
    assert ds.curr_status_info.status == DeploymentStatus.UPDATING
    assert (
        ds.curr_status_info.status_trigger
    ...
    # The new replica should start without waiting for the old one to stop.
    dsm.update()
    check_counts(
        ds,
        total=2,
    ...
    # New replica should start without waiting for old replica to stop
    dsm.update()
    check_counts(
        ds,
        total=2,
...
def test_redeploy_different_num_replicas(mock_deployment_state_manager_full):
    ...
    updating = dsm.deploy(TEST_DEPLOYMENT_ID, b_info_3)
    assert updating
    ...
    updating = dsm.deploy(TEST_DEPLOYMENT_ID, b_info_4)
    assert updating
...
def test_recover_state_from_replica_names(mock_deployment_state_manager_full):
    ...
    new_dsm: DeploymentStateManager = create_deployment_state_manager(
...
def test_recover_during_rolling_update(mock_deployment_state_manager_full):
    ...
    dsm = create_deployment_state_manager()
    cluster_node_info_cache.alive_node_ids = {"node-id"}

    # Step 1: Create some deployment info with actors in running state
    info1, v1 = deployment_info(version="1")
    updating = dsm.deploy(TEST_DEPLOYMENT_ID, info1)
    ds = dsm._deployment_states[TEST_DEPLOYMENT_ID]
    assert updating
    ...
    updating = dsm.deploy(TEST_DEPLOYMENT_ID, info2)
    assert updating
    ...
    new_dsm = create_deployment_state_manager(
        [ReplicaName.prefix + mocked_replica.replica_tag]
    )
    cluster_node_info_cache.alive_node_ids = {"node-id"}
...
def test_actor_died_before_recover(mock_deployment_state_manager_full):
    ...
    * In the following control loop update cycle, the controller adds a
    ...
    info1, v1 = deployment_info(version="1")
    updating = deployment_state_manager.deploy(TEST_DEPLOYMENT_ID, info1)
    deployment_state = deployment_state_manager._deployment_states[TEST_DEPLOYMENT_ID]
    assert updating

    # Single replica of version `version1` should be created and in STARTING state
    deployment_state_manager.update()
    check_counts(deployment_state, total=1, by_state=[(ReplicaState.STARTING, 1, v1)])
    ...
    mocked_replica._actor.set_ready()
    deployment_state_manager.update()
    check_counts(deployment_state, total=1, by_state=[(ReplicaState.RUNNING, 1, v1)])
    ...
    check_counts(
        new_deployment_state, total=1, by_state=[(ReplicaState.STARTING, 1, v1)]
    )
...
def test_shutdown(mock_deployment_state_manager):
    ...
    updating = deployment_state_manager.deploy(TEST_DEPLOYMENT_ID, b_info_1)
    assert updating
    ...
    # Single replica should be created.
    deployment_state_manager.update()
    check_counts(deployment_state, total=1, by_state=[(ReplicaState.STARTING, 1, None)])
    deployment_state._replicas.get()[0]._actor.set_ready()

    # Now the replica should be marked running.
    deployment_state_manager.update()
    check_counts(deployment_state, total=1, by_state=[(ReplicaState.RUNNING, 1, None)])
    ...
    check_counts(deployment_state, total=1, by_state=[(ReplicaState.STOPPING, 1, None)])
...
def test_get_active_node_ids(mock_deployment_state_manager_full):
    ...
    deployment_state_manager.update()
    check_counts(deployment_state, total=3, by_state=[(ReplicaState.STARTING, 3, v1)])
    ...
    for mocked_replica in mocked_replicas:
        mocked_replica._actor.set_ready()
    deployment_state_manager.update()
    check_counts(deployment_state, total=3, by_state=[(ReplicaState.RUNNING, 3, v1)])
    assert deployment_state.get_active_node_ids() == set(node_ids)
    assert deployment_state_manager.get_active_node_ids() == set(node_ids)
    ...
    deployment_state_manager.update()
    check_counts(deployment_state, total=3, by_state=[(ReplicaState.STOPPING, 3, v1)])
