...
def check_sum_metric_eq(
    metric: str, expected: float, tags: Optional[Dict[str, str]] = None
) -> bool:
    metrics = requests.get("http://127.0.0.1:9999").text
    sum = 0
    added_metrics = []
    for line in metrics.split("\n"):
        if metric in line and contains_tags(line, tags):
            try:
    ...
    # For debugging
    if added_metrics:
        print(f"The following sum to {expected} for metric '{metric}' and tags {tags}:")
