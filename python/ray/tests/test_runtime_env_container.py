...
@pytest.mark.skipif(sys.platform != "linux", reason="Only works on Linux.")
@pytest.mark.parametrize("use_new_api", [True, False])
def test_log_file_exists(podman_docker_cluster, use_new_api):
...
@pytest.mark.skipif(sys.platform != "linux", reason="Only works on Linux.")
@pytest.mark.parametrize("use_new_api", [True, False])
def test_ray_env_vars(podman_docker_cluster, use_new_api):
    """Test ray.put and ray.get."""
...
def test_worker_exit_intended_system_exit_and_user_error(
    podman_docker_cluster, use_new_api
):
    """
...
@pytest.mark.skipif(sys.platform != "linux", reason="Only works on Linux.")
@pytest.mark.parametrize("use_new_api", [True, False])
def test_serve_basic(podman_docker_cluster, use_new_api):
