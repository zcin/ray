...
class LocustMaster(LocustClient):
                ...
                for i, stage in enumerate(stages):
                    ...
                    if run_time < prefix_time:
                        if i != cls.curr_stage_ix:
                            self.on_stage_finished(stages[cls.curr_stage_ix].duration_s)
    ...
    def on_stage_finished(self, duration_s: int):
        print("jotting down stats for stage", duration_s)
        stats_entry_key = ("", "GET")
        stats_entry = self.master_runner.stats.entries.get(stats_entry_key)

        self.stats_in_stages.append(
            PerformanceStats(
                p50_latency=self.get_latency_percentile_over_window(duration_s, 0.5),
                p90_latency=self.get_latency_percentile_over_window(duration_s, 0.9),
                p99_latency=self.get_latency_percentile_over_window(duration_s, 0.99),
                rps=stats_entry.current_rps,
            )
        )

    def get_latency_percentile_over_window(self, window_s: int, percentile: float):
        ...
        acceptable_timestamps.append(t - window_s)
        for i in range(1, 9):
            acceptable_timestamps.append(t - window_s - i)
            acceptable_timestamps.append(t - window_s + i)

        print("acceptable timestamps:", acceptable_timestamps)

        for ts in acceptable_timestamps:
            if ts not in stats_entry.response_times_cache:
                continue

            print("found", ts, "in", stats_entry.response_times_cache.keys())
            cached = stats_entry.response_times_cache[ts]
            return calculate_response_time_percentile(
                diff_response_time_dicts(
                    stats_entry.response_times, cached.response_times
                ),
                stats_entry.num_requests - cached.num_requests,
                percentile,
            )
    ...
    def run(self):
        ...
        stats_entry_key = ("", "GET")
        stats_entry = self.master_runner.stats.entries.get(stats_entry_key)
        print("returning final stats, the stats in stages were:", self.stats_in_stages)
